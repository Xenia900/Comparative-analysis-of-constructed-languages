...
def pos_esperanto(filestr):
    print("\nParts of speech\n")
    from esperanto_analyzer import MorphologicalSentenceAnalyzer
    analyzer=MorphologicalSentenceAnalyzer(filestr)
    analyzer.analyze()
    return analyzer.simple_results() 
...
def main_procces_Esperanto(my_st,name_document):
    ...
    # Find which part of speech is each word 
    pos=[]
    pos=pos_esperanto(st)
    dic_pos_esperanto=procces_pos(pos)
    tk_tree_view(dic_pos_esperanto)
...
'''
start_esp=time.time()
file_esperanto = open('C:/Users/XENIA/Desktop/DIPLO/Esperanto_Text/esperantobig4.txt','r',encoding="utf-8")
